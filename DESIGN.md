...
The News surface minted no sizes. Its jobs map onto existing steps: masthead h1 → Title; letter stamps and the close h2 → Headline; masthead line → Body-emphasis; letter body → Body-story; close line → Body-close; selvage-rail links → Label-nav; the letter's meta line, foot link, back link, newer/older links, and optional print captions → Label / `.caps`.
...
- **Bolt-band** (`0 14px 36px -14px rgb(0 27 39 / 0.55)`): the heavier throw for the largest laid object — the home hero band and the News masthead band. Still written inline on both; not yet a custom property.
